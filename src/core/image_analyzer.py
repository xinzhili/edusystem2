...
2. 忽略以下情况：
   ...
   - 模糊无法辨别的标记
3. 输出格式要求：
    将图中真正错题信息转为JSON格式,要求包含字段：
    - student_id (学号,数据类型为字符串,默认值为'2025005')
    - subject (学科)
    - grade (年级)
    - textbook_version (教材版本,数据类型为整数,默认值为5)
    - week_num (周次,数据类型为整数,默认值为2)   
    -image_id: "{image_id}"
    - wrong_q_sum (错题知识点和具体信息，格式为列表，每个元素包含以下字段：
        - weak_knowledge_point (错题知识点）
        - error_questions (错题记录,格式为列表,每个元素包含以下字段：
            - question_id ("{image_id}_题目ID",数据类型为字符串)
            - question (题目内容)   
            - correct_answer (正确答案)
            - student_answer (学生答案)))
返回前要求：
    -请只返回JSON格式的结果,不要包含任何其他文本
    -返回前请严格检查jason格式,一个字段和值都不能少,避免重复和遗漏.
...
"""
        ...
        # 4. 验证必要字段
        required_fields = ["subject", "wrong_q_sum"]
...
if __name__ == "__main__":
    # Example usage
    image_path = r"D:\vsc\edusystem\src\core\wrongquestion.pdf"  # 替换为你的图片路径
